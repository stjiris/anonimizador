...
const temp_documents = new Map<string, { document: ApiDocument; nlp?: any; expiresAt: number }>();
...
export function saveDocument(token: string, document: any, ttlSeconds = 3600, nlp?: any) {
    const expiresAt = Date.now() + ttlSeconds * 1000;
    temp_documents.set(token, { document, nlp, expiresAt });
...
export function getAndDeleteDocument(token: string) {
    ...
    temp_documents.delete(token);
    return { document: data.document, nlp: data.nlp };
}
