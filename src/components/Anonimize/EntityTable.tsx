...
export function EntityTable({ file }: { file: UserFile }) {
    ...
    const bar = useMemo(() => toolbar(file.pool, file), [file.pool, file])
    ...
    return <MaterialReactTable
        ...
        renderTopToolbarCustomActions={toolbar(file.pool, file)}
...
const toolbar = (pool: EntityPool, file: UserFile) => ({ table }: { table: MRT_TableInstance<Entity> }) => {
    let selectedeKeys = selectedIndexes(table).length
    return <div className="d-flex w-100">
        <Button i="union" text="Juntar" className="btn btn-primary my-0 mx-1 p-1" disabled={selectedeKeys <= 1} onClick={() => joinSelectedEntities(table, pool, file)} />
        <Button i="exclude" text="Separar" className="btn btn-warning my-0 mx-1 p-1" disabled={selectedeKeys === 0} onClick={() => splitSelectedEntities(table, pool, file)} />
