...
export default function AnonimizeTooltip(props: AnonimizeTooltipProps) {
    ...
    switch (selection.would) {
        case AddEntityDryRun.CHANGE_ARRAY:
            return <div style={style}>
                <div className="d-flex flex-column gap-1 bg-white p-1 border">
                    {props.entityTypes.map((t, i) => <span key={i} role="button" className='badge text-body' style={{ background: t.color }} onMouseDown={() => setType(props.pool!, sel, t, fileA!)}>{t.name}</span>)}
                </div>
            </div>;
        ...
        case AddEntityDryRun.CHANGE_TYPE:
            return <div style={style}>
                <div className="d-flex flex-column gap-1 bg-white p-1 border">
                    <span role="button" onMouseDown={() => removeType(props.pool!, sel, fileA)}><i className='bi bi-trash'></i> Remover</span>
                    {props.entityTypes.map((t, i) => <span key={i} role="button" className='badge text-body' style={{ background: t.color }} onMouseDown={() => setType(props.pool!, sel, t, fileA!)}>{t.name}</span>)}
                </div>
            </div>;
        default:
