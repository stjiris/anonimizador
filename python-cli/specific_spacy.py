...
EXCLUDE = [
    ...
    "Exª",
    "Exº",
    "Secção do Supremo Tribunal de Justiça",
    "A.A.",
    "nºs",
]
...
def exclude_manual(ents):
    ...
    for e in ents:
        ...
        if text.lower().strip() in EXCLUDE:
            ...
            len(text) <= 1
        ):  # Changed from 2 to 1 to accommodate acronyms for political parties;
            continue
    ...
    for m in p.finditer(text):
        ...
        for e in ents:
            if (
                ...
                and start_pos <= e.end_char
                or end_pos >= e.start_char
                and end_pos <= e.end_char
            ):
                go = False
                break
        if go:
...
# Labels specified instances as X-* (non-anonymizable) entities
def label_X_entities_and_addresses(ents):

    for ent in ents:

        # For locations (entities of type "LOC") - convert to X-LOC
        # Note: Addresses are now detected by model as ADDR (mapped to MOR)
        if ent.label_ == "LOC":
            ent.label_ = "X-LOC"
    ...
    for ent in ents:
        ...
        if ent.label_ == "RED":
            ...
            if match and len(ent.text) >= 1:
                newEnts.append(
                    FakeEntity("RED", ent.start_char, ent.end_char, ent.text)
                )
...
def label_professions(doc, ents):
    ...
    pattern = [
        {"LOWER": {"IN": new_professions}}
    ]  # , {"TEXT": {"REGEX": r"\b(\w+)(a|o|as|os)?\b"}}]
...
def process_entities(ents, text):
    with open("patterns.csv", "r") as csvfd:
        ...
        for r in reader:
            # Skip IDP patterns - model now detects IDP directly
            if r["Label"] != "IDP":
...
def add_missed_entities(ents, text):
    ...
    for match in matches:
        ...
        if not any(
            old_start <= start <= old_end or old_start <= end <= old_end
            for old_start, old_end in added_spans
        ):
...
def merge(ents, text):
    ...
    for ent in ents:
        ...
        if str(last_ent.label_) == "LOC" and all(
            not s.isalnum() and not s == "\n"
        ...
        elif str(last_ent.label_) == "ORG" and all(
            s.isspace() or s == "-" and not s == "\n"
            for s in text[last_ent.end_char : ent.start_char]
        ):
            last_ent.end_char = max(ent.end_char, last_ent.end_char)
            last_ent.text = text[last_ent.start_char : last_ent.end_char]
    ...
    try:
        ...
        for chunk, position in zip(text_chunks[0], text_chunks[1]):
            ...
            for ent in exclude_manual(doc.ents):
                ...
                ents.append(
                    FakeEntity(ent.label_, ent.start_char, ent.end_char, ent.text)
                )
